...
class Cam1:
    def __init__(self, debug=True) -> None:
        ...
        self.frame = None
        if debug: self.cap = cv.VideoCapture(DEVICEDEBUGCAMERA1)
        else: 
            os.environ['OPENCV_FFMPEG_CAPTURE_OPTIONS'] = 'rtsp_transport;udp'
            self.cap = cv.VideoCapture(DEVICESTGCAMERA1, cv.CAP_FFMPEG)
        self.timeToCapture = None
        self.name = None
        self.captureThread = Thread(target=self.capture, name="CAPTURE")
        print("[]\tCAMERA 1 Starting.....")

    def stream(self):
        ret, self.frame = self.cap.read()
        if not ret:
            print('[]\t(CAMERA 1) Camera Module not detected')
        else:
    ...
    def stop(self):
        ...
        print("[]\tCAMERA 1 Stopping .....")

    def setTimeToCapture(self, t):
        if t == 0:
            self.timeToCapture = TIME
        else:
            print("[]\t CAMERA 1 capturing image delay for {} s".format(t))
            ...
            while True:
                ...
                if self.frame is not None:
                    if not cv.imwrite(filename=self.name, img=self.frame):
                        self.stop()
                        raise Exception('[]\t(CAMERA 1) Could not write image')
