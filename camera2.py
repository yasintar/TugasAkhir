...
class Cam2:
    def __init__(self, debug=True) -> None:
        ...
        self.frame = None
        if debug: self.cap = cv.VideoCapture(DEVICEDEBUGCAMERA2)
        else: 
            os.environ['OPENCV_FFMPEG_CAPTURE_OPTIONS'] = 'rtsp_transport;udp'
            self.cap = cv.VideoCapture(DEVICESTGCAMERA2, cv.CAP_FFMPEG)
        self.timeToCapture = None
        self.name = None
        self.captureThread = Thread(target=self.capture, name="CAPTURE")
        print("[]\tCAMERA 2 Starting.....")

    def stream(self):
        ret, self.frame = self.cap.read()
        if not ret:
            print('[]\t(CAMERA 2) Camera Module not detected')
        else:
    ...
    def stop(self):
        ...
        print("[]\tCAMERA 2 Stopping .....")

    def setTimeToCapture(self, t):
        if t == 0:
            self.timeToCapture = TIME
        else:
            print("[]\t CAMERA 2 capturing image delay for {} s".format(t))
    ...
    def capture(self):
        if self.timeToCapture is not None:
            while True:
                ...
                self.name = "./image/{}.jpg".format(now)
                if self.frame is not None:
                    if not cv.imwrite(filename=self.name, img=self.frame):
                        self.stop()
                        raise Exception('[]\t(CAMERA 2) Could not write image')
